...
require "./assets/sh.rb"; %x(chmod u+x /usr/bin/gamegrid)
...
#===== Variable Declarations ===
    ...
    answer3=ask("Type here your command (answer):").chomp
...
#File.open('~/.gg_internals/welcome.html', 'w') do |f2|  
  # use "\n" for two lines of text  
#  f2.puts "<!DOCTYPE html><html><body>Hello, we see that you've now visited GameGrid Internals. This file 'welcome.html' was stored in '~/.gg-internals/', the official folder of GameGrid gadask("Type here your command (answer):"), tools, and internal algorithms. Feel free to mess around with everything, but be slightly cautious about what you do. Take a look around! <br><br>Thanks, The GG Team</body></html>"
#end  
#end
rescue
file_main=false
...
if ($tutorialon===true) then
    ...
    command=ask("Type here your command (answer):")
    eval(command)
        if $loc.x===2 then
    ...
    if (p2over===true) then
        ...
        command=ask("Type here your command (answer):").chomp+""
    ...
    if (p3over===true) then
        ...
        command=ask("Type here your command (answer):").chomp; p4over=false;
        eval(command)
    ...
    if (p4over===true) then
        ...
        command=ask("Type here your command (answer):"); p5_1over=false;
    ...
    if (p5_1over===true) then
        ...
        command=ask("Type here your command (answer):"); p5_2over=false; 
    ...
    if (p5_2over===true) then
        ...
        nm=ask("Type here your command (answer):").chomp
...
if ($dimension==1) then
    ...
    if ($sector==1) then
        ...
        while($loc.x!=4||$loc.y!=2)
            command=ask("Type here your command (answer):"); eval(command); 
    ...
    if ($sector==2) then
        ...
        entered=ask("Type here your command (answer):"); if (entered.index("e")!=nil) then; ifenteredfac=true; end;
        if (ifenteredfac==true) then
            ...
            if (command.index(":eye")!=nil) then
                ...
                command=ask("Type here your command (answer):"); if command.index(":circuitboard")!=nil then; facdestroyedd=true; else; puts "Oh no! You mistyped! The computer broke down. Type play to restart Sector 2, and use the answers you know to get back, and oh, remember to type correctly! :D"; end;
    ...
    if ($sector==3) then
        ...
        command=ask("Type here your command (answer):"); $weapon=:iron_sword if (command.index("pickup")!=nil&&command.index(":iron_sword")!=nil);
 ...
 puts "You're floating at altitude "+$altitude.to_s+"! Good, now let's get him well. He has 7 lives, and an iron sword, too. But don't worry, we'll defeat him. Use the evoke command on white magic to heal yourself. Let's do this! Let's creep up on him while he naps. Walk to 3,3, cause that's where he is. You are currently standing at 6,2. You need to walk to 3,3. Go! "
                ...
                if ($loc.x==3&&$loc.y==3) then
                    ...
                    if $battle['opponent']['lives']<7 then
                        ...
                        if $battle['opponent']['lives']<5 then
                            ...
                            puts "Good! The opponent still stands strong. He gets his fists ready for another whapping! Quick, do something! Either, you can heal yourself, with white magic, or attack him! What do you choose? Do it!";         command=ask("Type here your command (answer):"); eval(command)
    ...
    if ($sector==5) then
        ...
        typeanything=ask("Type here your command (answer):"); yqn=1; wrongsentence="Yuni:' THAT'S WRONG! HA! HA! '                                  What a mean guy. Just type play and hit enter, to retry."
        if (yqn==1) then
            ...
            answer=ask("Type here your command (answer):").chomp.to_s; 
            yqn=2 if answer=="a";
...
#StartUp    
play() unless $tutorialcomp==false;
tutorial() unless $tutorialcomp==true;